				...
				for(var d=1;d<=30;d++){
					for(var i=0;i<10;i++){
						if(data.frequency[i]=="Monthly"){
							...
							if((m==data.month[i]||m==data.month[i]+3||m==data.month[i]+6||m==data.month[i]+9||m==data.month[i]-3||m==data.month[i]-6||m==data.month[i]-9)&&(d==data.day[i])){	
								saving_amount-=data.amount[i];
							}
						}
						else{
							if(m==data.month[i]&&d==data.day[i]){
								saving_amount-=data.amount[i];
							}
						}
					}
